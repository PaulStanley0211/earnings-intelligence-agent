...
| **4. Upload intake + transcript analyzer (complete)** | PDF/text intake, document advisor (ticker → EDGAR checklist), transcript Q&A extraction, answer classifier, commitment extractor, `POST /api/upload` and `POST /api/advise`, eval-mode watcher gated behind `WATCHER_MODE_ENABLED` | 75% F1 on 50 labeled Q&A pairs (uploaded transcripts); commitments persist across two consecutive quarters; document advisor returns correct latest filing on 5 test ticker/date pairs; `POST /api/upload` produces a `FilingEvent` and runs the existing pipeline end-to-end |
| **5a. Memory writes (complete)** | Persistent writes after every event (commitment-status open→met/missed transitions already landed in Phase 4B; Phase 5a inherits the residual write-after-every-event scope) | Multi-quarter synthetic run closes prior commitments correctly |
| **5b. Peer reader (complete)** | Memory-backed peer read-throughs (decide at kickoff whether the upload-first model warrants de-scoping) | Read-throughs cite real peer commentary verifiable in memory |
| **5c. Full critic (complete)** | Deterministic number checks + LLM fact-checking | Catches 90% of seeded errors in 30 adversarial notes; live intervention rate sits in the 5–20% band |

...
The data block below contains the facts, language changes, Q&A pairs,
management commitments, and peer commentary the critic will accept. Every
number AND every quoted phrase in your note must appear in the data block
and must be cited with the matching identifier:

- `[F#]` for a financial fact,
- `[C#]` for a comparison vs consensus,
- `[L#]` for a quoted language change,
- `[Q#]` for an analyst Q&A pair,
- `[K#]` for a management commitment, and
- `[P#]` for a peer commentary entry.
...
6. Do not invent metrics, ratios, growth rates, language changes, Q&A
   exchanges, commitments, or peer signals that are not in the data block.
   If you cannot derive a sentence from the supplied data, omit the sentence.
7. Output format: GitHub-flavored markdown. No headers above level 2.
   ...
   - `## Peer context`: zero to three bullets, each citing `[P#]`, drawing
     only on entries in `<source name="peers">`. Omit the entire section
     if the peers block is empty or contains no entries.
